...
namespace PaymentGateway.API.Idempotency
{
    public class IdempotencyFilter : Attribute, IAsyncResourceFilter
    {
        ...
        {
            _logger.LogInformation($"----> REUSING IDEMPOTENT RESPONSE - key: {response.Key} / ownerId: {response.OwnerId}");
            context.Result = new ObjectResult(JsonSerializer.Deserialize<object>(response.Body))
            {
